...
- **`tutor_sessions.escalation_reason` ya existe en producción, sin archivo de migración que lo cree** (descubierto 2026-07-30 reconciliando `supabase/migrations/` contra `schema_migrations` — ver `supabase/migrations/GAPS.md` y `known-drift.json`, entrada `20260610092119`). Columna `text` nullable, aplicada fuera de banda junto con `outcome`/`exercise_index` (migración 054, también parcialmente aplicada — falta su política RLS `students_read_own_sessions`, ver `docs/deuda-tecnica.md`). Al implementar la escalada en Fase 2, decidir explícitamente si esta columna se reutiliza tal cual o se rehace con otro diseño — y en cualquiera de los dos casos, escribir la migración que documenta lo que ya existe en producción (aunque no la cree, con `IF NOT EXISTS`, mismo patrón que 054), para que `known-drift.json` deje de tener que explicarla como excepción.
