...
export async function run({ test }) {
  ...
  test("el catálogo ofrece los temas con sus seis objetivos, cada uno con sus baterías y su nombre", () => {
    ...
    // curso.
    assert.deepEqual(c.temas.map((x) => x.nombre), ["Números enteros", "Potencias y raíces", "Divisibilidad", "Números decimales", "Fracciones", "Proporcionalidad y porcentajes", "Álgebra"]);
    const [t] = c.temas;
