...
export async function run({ test, assert }) {
  ...
  test("todavía no ha fichado entrada hoy -> el banner se muestra con el botón 'Fichar entrada'", async () => {
    const { el } = createFicharBanner({
    ...
    assert.equal(el.querySelector("button").textContent, "Fichar entrada");
  });
