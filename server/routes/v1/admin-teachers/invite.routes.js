...
// POST /admin/teachers/invite — extraído de admin.teachers.routes.js (era
// un único archivo de 553 líneas). Un profesor de instituto siempre
// necesita al menos un grupo/clase (concepto de curriculum: grupos +
// asignaturas); un profesor de academia no tiene ese concepto en absoluto
// (academia_alumnos usa un campo "curso" libre, no una tabla groups), así
// que la exigencia de group_ids se salta por completo para tenant.type
// === "academia" — el resto de la validación (InviteSchema ya tiene
// subjects/group_ids con default([])) ya era opcional.
export default async function adminTeachersInviteRoutes(app) {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (debeExigirGrupo(auth.tenant.type) && !groupIds.length) {
