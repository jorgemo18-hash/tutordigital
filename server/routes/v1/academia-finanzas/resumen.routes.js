...
const AnioPeriodoQuerySchema = AnioQuerySchema.extend({
  mes: z.coerce.number().int().min(1).max(12).optional(),
  trimestre: z.coerce.number().int().min(1).max(4).optional(),
});

async function autorizarYParsearAnio(req, reply, requestId, schema = AnioQuerySchema) {
...
export default async function academiaFinanzasResumenRoutes(app) {
  ...
  // GET /api/v1/academia/finanzas/resumen/fiscal?anio=&mes=&trimestre=
  // mes/trimestre son opcionales y mutuamente excluyentes (los aplica el
  // frontend); sin ninguno, se calcula el año completo.
  app.get("/fiscal", { preHandler: guard.preHandler }, async (req, reply) => {
    const requestId = req.requestId || makeRequestId();
    const auth = await autorizarYParsearAnio(req, reply, requestId, AnioPeriodoQuerySchema);
    if (!auth.ok) return;

    const admin = createSupabaseAdmin();
    const { fiscal, error } = await fetchResumenFiscal(admin, auth.tenantId, auth.anio, {
      mes: auth.mes, trimestre: auth.trimestre,
    });
    if (error) {
